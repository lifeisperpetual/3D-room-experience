...
scene.add(createBedsideLamp(-2.2, -2.1));
scene.add(createBedsideLamp(2.2, -2.1));
...
// Lamp lights (keep for local glow)
const lampLight1 = new THREE.PointLight(0xfff2cc, 1.2, 4);
lampLight1.position.set(-1.7, 1.2, -2.1);
scene.add(lampLight1);
const lampLight2 = lampLight1.clone();
lampLight2.position.set(1.7, 1.2, -2.1);
scene.add(lampLight2);

// Store lamp lights for animation
const lampLights = [lampLight1, lampLight2];
...
const tick = () => {
    ...
    lampLights.forEach(light => {
        light.intensity = baseIntensity * pulse * lampFade;
    });
...
// Curtains for window (static, matching size and shape)
const curtainWidth = 1.2;
const curtainHeight = 2.0; // extended for both
const curtainSegmentsW = 16;
const curtainSegmentsH = 32;
const curtainMat = new THREE.MeshStandardMaterial({ color: 0xffffff, side: THREE.DoubleSide, transparent: true, opacity: 0.92 });

// Left curtain (matches right)
const curtainGeoL = new THREE.PlaneGeometry(curtainWidth, curtainHeight, curtainSegmentsW, curtainSegmentsH);
for (let i = 0; i < curtainGeoL.attributes.position.count; i++) {
    const y = curtainGeoL.attributes.position.getY(i);
    // Curve: bulge outwards (same as right)
    const curve = -Math.sin((y / curtainHeight) * Math.PI) * 0.18;
    curtainGeoL.attributes.position.setZ(i, curve);
}
curtainGeoL.attributes.position.needsUpdate = true;
const curtainL = new THREE.Mesh(curtainGeoL, curtainMat);
curtainL.position.set(-3.88, 1.8, -0.7);
curtainL.rotation.y = Math.PI / 2;
scene.add(curtainL);

// Right curtain
const curtainGeoR = new THREE.PlaneGeometry(curtainWidth, curtainHeight, curtainSegmentsW, curtainSegmentsH);
for (let i = 0; i < curtainGeoR.attributes.position.count; i++) {
    const y = curtainGeoR.attributes.position.getY(i);
    // Curve: bulge outwards
    const curve = -Math.sin((y / curtainHeight) * Math.PI) * 0.18;
    curtainGeoR.attributes.position.setZ(i, curve);
}
curtainGeoR.attributes.position.needsUpdate = true;
const curtainR = new THREE.Mesh(curtainGeoR, curtainMat);
curtainR.position.set(-3.88, 1.8, 0.7);
